...
| 全量行业取数、成交额排序与前 30 筛选 | `src/market/eastmoney.ts` |
...
在测试群中发送 `@机器人 大盘`，能够在 5 分钟内收到**两条消息**：先是一条成交额 TOP30 文字榜单，再是一张 A 股行业板块图片；图片包含按成交额降序筛选的前 30 个行业，并标注东方财富数据源和行情时间；重复事件不会重复回复；数据源或图片生成失败时能够返回文字错误提示。
