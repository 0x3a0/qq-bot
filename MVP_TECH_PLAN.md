...
- 排序后只取前 25 个行业板块用于生成图片。
- 对行情结果设置短时缓存，图片上标注“东方财富”和 `f124` 对应的行情时间。
- 东方财富接口属于公开网页行情接口，没有稳定性保证；MVP 暂不接入备用数据源。

#### 行业 / 概念板块资金流（已探明接口，实测可用）

同一接口的 `clist/get` 换用 `fid=f62` 即返回资金流排行，`f62` 为「主力净流入额」（单位：元）：

| 参数 | 行业板块 | 概念板块 |
|---|---|---|
| `fs` | `m:90 t:2`（实测 496 个） | `m:90 t:3`（实测 504 个） |
| `fid` | `f62` 今日 / `f164` 5日 / `f174` 10日 | 同左 |

关键字段：`f62` 主力净额、`f66` 超大单、`f72` 大单、`f78` 中单、`f84` 小单，
占比分别为 `f184`/`f69`/`f75`/`f81`/`f87`；5 日与 10 日为整段平移
（`f164`~`f173`、`f174`~`f183`）。`f204`/`f205` 为领涨股。

单板块分钟级资金流走 `GET /api/qt/stock/fflow/kline/get`，`secid=90.BKxxxx&klt=1`。

实测校验：分页结果数与 `total` 完全一致（496/504），且 `f62 = f66 + f72` 在全部板块上严格成立。
完整字段表、主机可用性对比与踩坑记录见 [README](./README.md#东方财富资金流接口行业--概念)；
代码入口为 `src/market/fundflow.ts`，自检命令 `npm run verify -- fundflow [today|5d|10d]`。
